  ...
  return (
   <Loading isLoading={isLoading}>
     <div style={{width: '100%', height: 'auto', background: '#f5f5fa'}}>
      ...
      <h4 style={{marginTop: '0px', paddingTop: '9px'}}>Chi tiết đơn hàng</h4>
        <WrapperHeaderUser>
          <WrapperInfoUser>
        ...
        <Divider/>
        <WrapperStyleContent>
          <div style={{flex:1,display: 'flex', alignItems: 'center', justifyContent: 'space-between',   paddingLeft: '10px'}}>
          ...
          {data?.orderItems?.map((order) => {
            return (
              <WrapperProduct key={order?.id}>
                <WrapperNameProduct>
                  <img alt={order?.id} src={order?.image} 
                    style={{
                      width: '70px', 
                      height: '70px', 
                      objectFit: 'cover',
